...
document.getElementById('preloadButton').addEventListener('click', () => {
  for (let i = 0; i < filePaths.length; ) {
    ...
    /* geometry.position.x = (i % 8) * 20 - 10
    geometry.position.y = Math.floor(i / 8) * 20 - 10
...
function runThreeJS() {
  ...
  //Add Event Listener
  // Listen for scroll and move y position of camera to simulate perspective
  ...
  function updateCamera(selected) {
    ...
    title.innerHTML = filePaths[index].title

    let artist = document.createElement('span')
    // first character upper case
    artist.innerHTML =
      '~ ' +
      filePaths[index].artist.charAt(0).toUpperCase() +
      filePaths[index].artist.slice(1)

    let button = document.createElement('button')
    button.classList.add('btn', 'btn-primary', 'rounded')
    button.style.marginTop = '50px'
    button.innerHTML = `<img src="./images/Ethereum_logo.svg" width="20" style="margin-right: 10px;"> Available on OpenSea`
    button.addEventListener('click', () => {
      window.open(filePaths[index].link)
    })
    ...
    setTimeout(() => {
      ...
      document.querySelector('.textContianer').appendChild(infoContainer)
    }, 2200)
  }
}
